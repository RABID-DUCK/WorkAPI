<?php 
require_once dirname(__DIR__, 1).'/controller/controller_listVersions.php';
$arr = new Controller_ListVersions();

if(isset($arr->list)){

foreach ($arr->list["payload"]["array"] as $row) {
    echo $html = '<tr><td class="id-version">'.$row["id"].'</td>
<td>
    <div class="d-flex align-items-center">
        <div class="d-flex justify-content-start flex-column">
            <a href="#"
                class="text-dark fw-bolder text-hover-primary fs-6">'.$row["major"].'.
                '.$row["minor"].'.'.$row["micro"].'</a>
        </div>
    </div>
</td>
<td>
    <a href="#"
        class="text-dark fw-bolder text-hover-primary d-block fs-6">'.$row["releaseNote"].'</a>
</td>
<td>
